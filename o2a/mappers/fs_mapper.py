...
def prepare_mkdir_command(node: Element, props: PropertySet):
    path = normalize_path(node.attrib[FS_TAG_PATH], props=props, translated=True)
    command = "hdfs dfs -mkdir -p {path}".format(path=shlex.quote(path))
    return command


def prepare_delete_command(node: Element, props: PropertySet):
    path = normalize_path(node.attrib[FS_TAG_PATH], props=props)
    command = "hdfs dfs -rm -f -r {path}".format(path=shlex.quote(path))
...
def prepare_move_command(node: Element, props: PropertySet):
    ...
    command = "hdfs dfs -mv {source} {target}".format(source=shlex.quote(source), target=shlex.quote(target))
    return command
...
def prepare_chmod_command(node: Element, props: PropertySet):
    ...
    command = "hdfs dfs -chmod {extra} {permission} {path}".format(
...
def prepare_touchz_command(node: Element, props: PropertySet):
    ...
    command = "hdfs dfs -touchz {path}".format(path=shlex.quote(path))
    return command
...
def prepare_chgrp_command(node: Element, props: PropertySet):
    ...
    command = "hdfs dfs -chgrp {extra} {group} {path}".format(
...
class FsMapper(ActionMapper):
    ...
    def parse_fs_operation(self, index: int, node: Element, operation_nodes_count: int) -> Task:
        ...
        dfs_command = mapper_fn(node, props=self.props)
        return Task(
            task_id=task_id,
            template_name="fs_op.tpl",
            template_params=dict(
                dfs_command=dfs_command, action_node_properties=self.props.action_node_properties
            ),
        )
